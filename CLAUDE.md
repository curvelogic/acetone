...
- **Explicit-instruction delegation (ADR-0067)**: an explicit, informed, current instruction from Greg authorises an agent to perform an act these documents otherwise reserve for him — closing a gate/exit-criteria bead, publishing a release. The instruction is recorded verbatim where the act is recorded (bead close reason, release record), never normalised silently. Absent such an instruction the reservations stand; ambiguity or silence is not consent, and a past delegation does not carry forward to the next occasion.
