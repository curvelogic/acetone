...
- `docs/acetone-03-roadmap.md` — phased implementation plan (Phase 0–10) with exit criteria and decision gates
...
- **Follow-ups are resolved within the phase by default** (ADR-0054). Work that surfaces mid-flight is captured as a follow-up only when it is worth a real future unit of effort — a genuinely trivial nit is fixed inline or dropped, not filed. The phase that surfaces a follow-up closes it before the phase closes, unless it is genuinely *out of scope* or *depends on later-phase work*; "it's only P3" is not a reason to cross the boundary. Any follow-up that does cross is named in the phase report **with the reason it could not be resolved in-phase** and re-homed to an owning epic — never left floating under the closed phase's epic. A corollary: **a user-facing feature is not delivered until it is reachable through the shipped interface**, so its CLI/API surface is in-scope for the phase that ships it, not a follow-up (a purely internal capability is exempt — its "interface" is the code that uses it).
...
- **Phase boundary**: agents never close the bead that gates the next phase on their own initiative (the exit-criteria bead; for Phase 0, the Gate A decision bead; see explicit-instruction delegation below). The agent completing the phase's last working bead writes `docs/reports/phase-N.md` — what shipped, gate evidence against the roadmap's exit criteria, ADRs taken, review findings summary, open risks, and any follow-up that crosses the boundary with the reason it could not be resolved in-phase (ADR-0054) — and stops. The boundary review then runs as a **sprint demo** (Greg's preferred format, set 2026-07-04): (1) a several-slide presentation (Artifact, archived to `docs/demos/`) covering context, where we are, what was done, and the problems and decisions that arose; (2) a live demo — driving the phase's actual code/CLI/tooling **step by step, one step per turn**: announce the step, run it, quote the command and output in the message text, explain what it means, and stop for discussion before proceeding (never batch steps into background calls or one summary); (3) Greg reviews the docs in detail at his own pace. Greg then discusses, rules on queued decisions, and closes the gate bead — himself, or by explicit-instruction delegation (ADR-0067); the next phase's beads unblock from there.
- **Phase start** (working agreement, Greg 2026-07-24; recorded here 2026-08-01): agents claim a phase's implementation beads only after Greg has explicitly opened the phase — the previous phase's gate close unblocks the beads, but it is not itself the opening. Between boundaries sessions do spec and grooming — discussion, ADRs, homing beads into epics — without implementing; a ratified design decision is not a start signal.
...
- **Explicit-instruction delegation (ADR-0067)**: an explicit, informed, current instruction from Greg authorises an agent to perform one of the **two acts** these documents reserve for him — closing a gate/exit-criteria bead, or publishing a release — **and no others**; extending that set is itself a governing-doc change requiring Greg's agreement. The instruction must be received **first-hand from Greg in the current session** — a relayed or reported instruction (e.g. via another agent) is not a delegation and must be confirmed with Greg directly. It is recorded **verbatim** where the act is recorded (the bead close reason; for a release, the publish step's bead), never normalised silently. Absent such an instruction the reservations stand; ambiguity or silence is not consent, and a past delegation does not carry forward to the next occasion. Agents may say delegation is possible if Greg asks how to proceed, but must never solicit it.
...
- **Phase exit criteria are review points**: the exit-criteria bead of each phase is closed by Greg after reviewing the gate evidence — himself, or by an agent acting on his explicit, informed instruction with the instruction recorded in the close reason (ADR-0067); never on an agent's own initiative.
