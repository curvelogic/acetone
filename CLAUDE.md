...
- **Phase boundary**: agents never close the bead that gates the next phase (the exit-criteria bead; for Phase 0, the Gate A decision bead). The agent completing the phase's last working bead writes `docs/reports/phase-N.md` — what shipped, gate evidence against the roadmap's exit criteria, ADRs taken, review findings summary, open risks — and stops. The boundary review then runs as a **sprint demo** (Greg's preferred format, set 2026-07-04): (1) a several-slide presentation (Artifact) covering context, where we are, what was done, and the problems and decisions that arose; (2) a live demo — driving the phase's actual code/CLI/tooling in-session with commentary; (3) Greg reviews the docs in detail at his own pace. Greg then discusses, rules on queued decisions, and closes the gate bead; the next phase's beads unblock from there.
