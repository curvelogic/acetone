...
Acetone is an embedded, single-node, version-controlled labelled property graph database with an openCypher query interface and a git-compatible storage backend. It is a **solo, git-native workbench for a version-controlled asset registry**: invoked as a CLI within a repository directory (not operated as a network service), where imports become audited commits, `diff` becomes a change report, and any git remote is backup and transport. In v0.1 the **CLI is the product**; the `acetone-core` library API is **frozen at 0.2** (ADR-0046; the gate — the query-engine resource governor — shipped in Phase 7). Out of scope for this specification: multi-writer concurrency, network query protocols (Bolt etc.), horizontal scale, columnar analytics performance, and the view/virtual/RDF/relational projection layers (reserved in §9 so their hooks are stable).
...
`schema` — label definitions, key declarations, constraints, index declarations. `nodes` — node key → node record (secondary labels, properties). `edges_fwd` — (src key, type, dst key, disc) → edge record (properties). `edges_rev` — (dst key, type, src key, disc) → empty. `idx/<name>` — declared property indexes, `(label, [property names], [encoded values], node key)` → empty; a **composite** index over more than one property keys on the ordered value tuple, a single-property index on a one-element tuple (ADR-0027). `conflicts` — present only in a merge-in-progress workspace; see §6.
...
Parser (decypher or equivalent, producing a spanned AST) → binder (resolves labels, keys, indexes against `schema`) → executor. **In v0.1 the executor is a clause pipeline over materialised row sets, not a reified tree of iterator operators** — at workbench scale (§1) streaming buys nothing yet, so the logical operators below (`LabelScan`, `KeySeek`, `IndexSeek/Range`, `ExpandOut`/`ExpandIn`, `Filter`, `Project`, `Aggregate`, `Sort`, `Limit`, `VarExpand`, `Apply` for `OPTIONAL MATCH`/pattern predicates, `Create/Merge/Set/Delete` sinks) live as the pattern-matcher's internals and clause handlers rather than as standalone `Operator` structs. Planning is likewise implicit and heuristic (the matcher's anchor choice: a bound variable or the most selective scan available — use an index when one covers a predicate); a costed planner and a lazy iterator model are deferred (the seam for a store-backed `IndexSeek` is `acetone-cbl.11`). Null semantics follow openCypher exactly and are TCK-verified.
...
`acetone init [--object-format sha256] | status | log | diff [<a> [<b>]] | branch | checkout <ref> | commit -m | merge <ref> | resolve | query '<cypher>' [--at <ref>] [--format table|json|csv] | shell | import --format <plugin> <source> [--branch] | export --format <fmt> | reindex | gc | fsck`. Backup and transport are plain git on the enclosing repository — `git push`/`git pull`/`git clone` (§3.5); a remote need not know acetone exists, so acetone has no push/pull/clone subcommands of its own. `shell` is a readline Cypher REPL with `:checkout`, `:log`, `:format` conveniences. `fsck` verifies chunk reachability, manifest integrity, edge-map symmetry, index consistency and history independence spot-checks. **In v0.1 the CLI is the product surface**; the same operations are also reachable through the `acetone-core` Rust library, whose curated public API is **frozen at 0.2** (ADR-0046; `STABILITY.md`) now the gating query-engine resource governor has shipped, keeping a stable surface for library/agent frontends and editor integrations.

## 8. Crate layout

`acetone-store` (ChunkStore trait; git backend; refs/commits) · `acetone-prolly` (trees, diff, merge; property-tested for history independence) · `acetone-model` (keys, records, encodings, schema, manifest) · `acetone-graph` (graph mutations, constraints, validation, merge orchestration) · `acetone-cypher` (front end, heuristic planner, executor; TCK harness) · `acetone-core` (the façade re-exporting the above as the single library API, whose curated public surface is frozen at 0.2 — ADR-0046, ADR-0026) · `acetone-cli` (the CLI client — the v0.1 product surface). Dependency direction is strictly downward in that list.
...
Until 1.0, the manifest carries `format_version`; any change to key encoding, value encoding, chunking parameters or manifest schema increments it. **Gate D (ADR-0024) froze `format_version = 1` for the 0.1 release**: the §3.4 encodings, the prolly node/chunker format and the manifest schema are byte-anchored by golden tests (a change flips a pin, failing loudly rather than drifting silently), and the version is read first on every manifest decode.

Crossing a format boundary has two strategies, and **read-old-write-new is the default** (ADR-0048; the dispatch machinery is ADR-0052). Rather than rejecting any non-current version, `Manifest::decode` *dispatches* on `format_version` to a retained decoder per format the build has ever shipped: old commits stay readable through their era's decoder, new writes always use the current format, and **no existing commit is rewritten** — so one repository may hold commits at several versions side by side, and a graph sharing a repo with code never force-pushes rewritten history. A version with no retained decoder (a repository written by a *newer* build) is still rejected with a clear error rather than misread. `acetone migrate` — which rewrites history to bring every commit up to the current format, producing new hashes — remains available but is now the **opt-in** tool for standalone repositories, not the default evolution path.
